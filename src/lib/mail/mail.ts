...
export const sendRecoveryEmail = async (
    ...
    const link = `https://localhost:3000/recovery?id=${userId}&code=${recoverHash}`;
...
export const sendVerificationEmail = async (
    ...
    const link = `https://localhost:3000/api/verify?id=${userId}&code=${emailHash}`;
...
const sendFormattedEmail = async (
    ...
    // console.log(email, "---", subject, "---", rendered)
};

const sendEmail = async (to: string, subject: string, content: string) => {
    transporter.sendMail({
        from: "email@renodots.com",
        to,
        subject,
        html: content,
    });
