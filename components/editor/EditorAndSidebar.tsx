...
const EditorAndSidebar = ({ project }: Props) => {
    ...
    const setActiveTab = (node: Tab, applyStyle = true) => {
        ...
        if (editorView) {
            ...
            if ((e.ctrlKey || e.metaKey) && e.shiftKey) {
                ...
                if (e.key.toLowerCase() === "d") {
                    e.preventDefault();
                    editorView.isFocused
                        ? editorView.chain().blur().run()
                        : editorView.chain().focus().run();
                }
            ...
            if ((e.ctrlKey || e.metaKey) && e.altKey) {
                if (e.key.toLowerCase() === "x") {
                    e.preventDefault();
                    editorView.chain().focus().toggleStrike().run();
                }
            }
        }
    };
    ...
    const replaceOccurrences = (oldWord: string, newWord: string) => {
        editorView
            ?.chain()
            .focus()
            .insertContentAt({ from: 0, to: 4 }, newWord)
            .run();
    };
    ...
    return (
        <div className={editor_.editor_and_sidebar}>
            ...
            <div className={editor_.container} onScroll={onScroll}>
                <EditorComponent editor={editorView} project={project} />
            </div>
            {/* <EditorSidebarFormat
        tabs={tabs}
