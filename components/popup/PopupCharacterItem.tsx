...
const PopupCharacterItem = ({
  ...
  const [newGender, setNewGender] = useState<CharacterGender>(
        ...
        <form className={popup.form} onSubmit={def.onSubmit}>
          {newNameWarning && (
            <div className={join(popup.info, form_info.warn)}>
              <p>
                ...
                word {character?.name} to {newName}? Take extra care of common
                words whose update might be unwated.
              </p>
          ...
          {takenNameError && (
            <div className={join(popup.info, form_info.error)}>
              <p>
                A character with the name {newName} already exists. Please
                choose a different name or edit the existing character instead.
              </p>
            </div>
          )}
